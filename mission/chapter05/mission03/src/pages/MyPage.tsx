...
import type { ResponseMyInfoDto } from '../schemas/auth';

// ✅ 서버 DTO의 data 타입을 그대로 사용 (중복 타입 정의 X)
type UserInfo = ResponseMyInfoDto['data'];
...
const ProtectedPage = () => {
  ...
  useEffect(() => {
    const fetchUserInfo = async () => {
      try {
        const response = await getMyInfo();   // response: CommonResponse<UserInfo>
        setUserInfo(response.data);           // ✅ 타입 일치
      } catch (err: any) {
